...
function talkaction.onSay(player, words, param)
    ...
    -- Segurança: impede mexer em alguém com grupo maior
    if target:getGroup():getAccess() and not player:getGroup():getAccess() then
        player:sendCancelMessage("You cannot access this player.")
        return true
    end

    if storageValue == nil then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE,
            "[" .. target:getName() .. " - " .. storageKey .. "] = " .. target:getStorageValue(storageKey))
    else
        target:setStorageValue(storageKey, storageValue)
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE,
            "Set storage [" .. storageKey .. "] = " .. storageValue .. " for player " .. target:getName())
    end
